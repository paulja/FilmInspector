...
/// View model for the `FilmDetailView` view, added when selecting a cell from the film list.
class FilmDetailViewModel: ObservableObject {
    ...
    /// sets the model cache and then signals the view with the `isLoaded` published
    /// field.
    func load() {
